...
-- 6. Abilita Row Level Security (RLS) per public.sites
ALTER TABLE public.sites ENABLE ROW LEVEL SECURITY;

-- 7. Policy per 'sites' (con cast esplicito per massima sicurezza)
...
-- 8. Abilita Row Level Security (RLS) per public.tasks
ALTER TABLE public.tasks ENABLE ROW LEVEL SECURITY;

-- 9. Policy per 'tasks' (con cast esplicito per massima sicurezza)
...
-- 10. Crea la tabella public.users se non esiste
...
-- 11. Abilita Row Level Security (RLS) per public.users
...
-- 13. Funzione per creare un profilo utente nella tabella 'public.users' dopo la registrazione in auth.users
...
-- 14. Trigger per chiamare la funzione dopo l'inserimento in auth.users
